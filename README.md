...
- **Design write-up:** [WRITEUP.md](WRITEUP.md) — data model, the deadlock I
  shipped and then fixed, where idempotency lives, consistency-vs-availability.
- **Live URL:** _see below_
- **Live logs:** `<URL>/logs` · **Dashboard:** `<URL>/dashboard` · **Metrics:** `<URL>/metrics`
- **Live invariant audit:** `<URL>/invariants`
